...
class Trainer:
    ...
    def train(self, model: nn.Module, train_loader: DataLoader, val_loader: Optional[DataLoader] = None) -> Dict[str, List[float]]:
        """
        Treina o modelo
        ...
        for epoch in range(self.epochs):
            ...
            if val_loader:
                ...
                log.info(f"Época {epoch+1}/{self.epochs} - Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}")
            else:
    ...
    def split_dataset(self, data: List[Dict[str, Any]], train_ratio: float = 0.7, val_ratio: float = 0.15):
