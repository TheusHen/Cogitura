...
class Evaluator:
    ...
    def evaluate_on_dataset(self, model, db_manager) -> Dict[str, Any]:
        ...
        wer_accum = sum(self.calculate_wer(ref, hyp) for ref, hyp in zip(sentences, predictions)) / total
        return {
            "accuracy": accuracy,
            "wer": wer_accum,
    ...
    def generate_confusion_matrix(self, predictions: List[str], ground_truth: List[str]) -> np.ndarray:
        """
    ...
    def save_results(self, results: Dict[str, Any], filename: str = "evaluation_results.json") -> Path:
        """
    ...
    def calculate_metrics_summary(self, predictions: List[str], ground_truth: List[str]) -> Dict[str, Any]:
