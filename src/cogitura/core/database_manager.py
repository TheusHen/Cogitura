...
class DatabaseManager:
    ...
    def __init__(
        ...
        try:
            if hasattr(self.es, "ping") and not self.es.ping():
                log.warning(f"ElasticSearch não respondeu ao ping em {self.host}:{self.port} (ignorando em testes)")
        except Exception:
    ...
    def _increment_word_usage(self, word: str) -> None:
        ...
        # Atualiza contador
        self.es.update(
            index=self.words_index,
            id=word,
            body={"doc": {"usage_count": current_count + 1}}
        )
