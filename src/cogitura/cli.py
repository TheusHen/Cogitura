...
def generate(count, batch_size, save_db, generate_tts):
    ...
    if generate_tts:
        ...
        console.print(f"[bold green]TTS gerado: {tts_stats['total_files']} arquivos, {tts_stats['total_size_mb']:.2f} MB[/bold green]")


@main.command()
@click.option("--model", "-m", default="facebook/wav2vec2-base-960h", help="Modelo base para treinamento")
